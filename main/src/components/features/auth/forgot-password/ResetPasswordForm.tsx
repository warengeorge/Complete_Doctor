...
import { PasswordCheck } from "@/components/shared/icons/icons";
import { useAuthStore } from "@/lib/store/auth/auth.store";

export default function ResetPasswordForm() {
  const router = useRouter();
  const { resetEmail, resetPassword } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // once the backend is ready, this would call an API to verify OTP and reset password
      // simulate a successful request
      await new Promise((resolve) => setTimeout(resolve, 1000));

      // Update password in the auth store

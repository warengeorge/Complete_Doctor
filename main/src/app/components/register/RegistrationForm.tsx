...
export default function RegistrationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        // Store user details for the verification step
        setPendingRegistration({
          firstName: firstname,
          lastName: lastname,
          email,
        ...
        // Start registration process (send email)
        await registerStart(email);

        router.push("/verify-registration");
      } catch (err) {
        console.warn("Registration start failed", err);
      }
    }
  };
  ...
  return (
    <div className="w-[540px] max-w-md h-auto flex flex-col gap-10 justify-center px-5">
      ...
      {authError && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-[5px] text-sm">
          {authError}
        </div>
      )}
